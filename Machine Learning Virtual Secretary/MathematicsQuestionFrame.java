 ...
 *
 */
public class MathematicsQuestionFrame extends JFrame{
	 ...
	 */
	public MathematicsQuestionFrame(student User, Department department)
	{
		createLabels();
		answer = "";
		question = "";
		...
		setTitle("Mathematics Ask a Question");
	...
	class continueButtonListener implements MouseListener
	{
		...
		{
			findQuestion = new Question(question);
			
			try {
				findQuestion.checkExactMatch();

			} catch (FileNotFoundException e) {
				// TODO Auto-generated catch block
				System.out.println(e);
			}
			if (findQuestion.getExactMatch())
			{
				setVisible(false);
				exactMatchFrame = new exactAnswerFrame(findQuestion, User);
			}

			if (!findQuestion.getExactMatch())
			{
				try {
					findQuestion.GuessMatchFindKeyWords(); 
				} catch (FileNotFoundException e) {
					// TODO Auto-generated catch block
					System.out.println(e);
				}

				if (!findQuestion.getExactMatch() && findQuestion.getGuess())
				{
					setVisible(false);
					guessMatchFrame = new guessAnswerFrame(findQuestion, User, department);
				}

				if(!findQuestion.getGuess())
				{
	...
	{
		label1 = new JLabel("Ask a question for the Math Department");
		label1.setFont(new Font("Ask a question for the Math Department", Font.ITALIC, 25));
	...
	public JPanel createLowerPanel()
	{
		...
		lowerPanel.setBorder(new TitledBorder(new EtchedBorder(), "Mathematics"));
	...
	public void sendEmail()
	{
		...
		try {
			...
			message.setRecipients(
					...
					InternetAddress.parse("edwinlau20@gmail.com") //add user.getEmail()
					);
